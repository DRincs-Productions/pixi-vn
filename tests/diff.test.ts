import deepDiff from "deep-diff";
import { Difference } from "microdiff";
import { expect, test } from "vitest";
import { restoreDiffChanges } from "../src/utils/diff-utility";

test("restoreDeepDiffChanges & restoreMicroDiffChanges", async () => {
    let data = {
        path: "/narration",
        storage: [
            {
                key: "___current_dialogue_memory___",
                value: {
                    text: "I just wanted to see what the new guy was like.",
                    character: "sly",
                },
            },
            {
                key: "___last_dialogue_added_in_step_memory___",
                value: 14,
            },
            {
                key: "___flags___",
                value: [],
            },
            {
                key: "___opened_labels_counter___",
                value: {
                    start: {
                        biggestStep: 14,
                        openCount: 1,
                    },
                },
            },
            {
                key: "_input_value_",
                value: "Peter",
            },
            {
                key: "___last_input_added_in_step_memory___",
                value: 5,
            },
            {
                key: "___character___",
                value: {
                    mc: {
                        name: "Peter",
                    },
                },
            },
        ],
        canvas: {
            tickers: {
                "41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd": {
                    id: "MoveTicker",
                    args: {
                        direction: "right",
                        speed: 300,
                        tickerAliasToResume: ["sly"],
                        aliasToRemoveAfter: [],
                        destination: {
                            x: 0.2,
                            y: 1,
                            type: "align",
                        },
                        startOnlyIfHaveTexture: true,
                    },
                    canvasElementAliases: ["sly"],
                },
                "170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7": {
                    id: "MoveTicker",
                    args: {
                        direction: "left",
                        speed: 300,
                        tickerAliasToResume: ["steph"],
                        aliasToRemoveAfter: [],
                        destination: {
                            x: 0.8,
                            y: 1,
                            type: "align",
                        },
                        startOnlyIfHaveTexture: true,
                    },
                    canvasElementAliases: ["steph"],
                },
            },
            tickersSteps: {},
            elements: {
                bg: {
                    pixivnId: "Image",
                    elements: [],
                    width: 1920,
                    height: 1080,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 0,
                        y: 0,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 0,
                    y: 0,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    textureData: {
                        url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fbg01-hallway.webp?alt=media",
                        alias: "bg01-hallway",
                    },
                    anchor: {
                        x: 0,
                        y: 0,
                    },
                    roundPixels: false,
                    onEvents: {},
                    loadIsStarted: false,
                },
                james: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-body.webp?alt=media",
                                alias: "m01-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-eyes-annoy.webp?alt=media",
                                alias: "m01-eyes-annoy",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy00.webp?alt=media",
                                alias: "m01-mouth-annoy00",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 495,
                    height: 977,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 712.5,
                        y: 103,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 712.5,
                    y: 103,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    align: {
                        x: 0.5,
                        y: 1,
                    },
                    loadIsStarted: false,
                },
                sly: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-body.webp?alt=media",
                                alias: "fm01-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-wow.webp?alt=media",
                                alias: "fm01-eyes-wow",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-soft01.webp?alt=media",
                                alias: "fm01-mouth-soft01",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 546,
                    height: 980,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: -546,
                        y: 100,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: -546,
                    y: 100,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    loadIsStarted: false,
                },
                steph: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-body.webp?alt=media",
                                alias: "fm02-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-eyes-nervous.webp?alt=media",
                                alias: "fm02-eyes-nervous",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-mouth-nervous00.webp?alt=media",
                                alias: "fm02-mouth-nervous00",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 525,
                    height: 969,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 2445,
                        y: 111,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 2445,
                    y: 111,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    loadIsStarted: false,
                },
            },
            stage: {
                pixivnId: "Container",
                elements: [],
                width: 3516,
                height: 1080,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: 0,
                    y: 0,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: 0,
                y: 0,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
            },
            elementAliasesOrder: ["bg", "james", "sly", "steph"],
            tickersOnPause: {
                sly: {
                    tickerIdsExcluded: ["41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd"],
                },
                steph: {
                    tickerIdsExcluded: ["170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7"],
                },
            },
            tickersToCompleteOnStepEnd: {
                tikersIds: [
                    {
                        id: "41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd",
                    },
                    {
                        id: "170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7",
                    },
                ],
                stepAlias: [],
            },
        },
        sound: {
            soundsPlaying: {},
            soundAliasesOrder: [],
        },
        labelIndex: 14,
        openedLabels: [
            {
                label: "start",
                currentStepIndex: 14,
            },
        ],
    };
    let differencesOld: deepDiff.Diff<any, any>[] = [
        {
            kind: "E",
            path: ["storage", 3, "value", "start", "biggestStep"],
            lhs: 13,
            rhs: 14,
        },
        {
            kind: "E",
            path: ["storage", 1, "value"],
            lhs: 13,
            rhs: 14,
        },
        {
            kind: "E",
            path: ["storage", 0, "value", "text"],
            lhs: "I know you're both watching, come on out already!",
            rhs: "I just wanted to see what the new guy was like.",
        },
        {
            kind: "E",
            path: ["storage", 0, "value", "character"],
            lhs: "james",
            rhs: "sly",
        },
        {
            kind: "N",
            path: ["canvas", "tickers", "41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd"],
            rhs: {
                id: "MoveTicker",
                args: {
                    direction: "right",
                    speed: 300,
                    tickerAliasToResume: ["sly"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.2,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["sly"],
            },
        },
        {
            kind: "N",
            path: ["canvas", "tickers", "170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7"],
            rhs: {
                id: "MoveTicker",
                args: {
                    direction: "left",
                    speed: 300,
                    tickerAliasToResume: ["steph"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.8,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["steph"],
            },
        },
        {
            kind: "E",
            path: ["canvas", "elements", "james", "elements", 2, "textureData", "url"],
            lhs: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy01.webp?alt=media",
            rhs: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy00.webp?alt=media",
        },
        {
            kind: "E",
            path: ["canvas", "elements", "james", "elements", 2, "textureData", "alias"],
            lhs: "m01-mouth-annoy01",
            rhs: "m01-mouth-annoy00",
        },
        {
            kind: "N",
            path: ["canvas", "elements", "sly"],
            rhs: {
                pixivnId: "ImageContainer",
                elements: [
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 546,
                        height: 980,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-body.webp?alt=media",
                            alias: "fm01-body",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 546,
                        height: 980,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-wow.webp?alt=media",
                            alias: "fm01-eyes-wow",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 546,
                        height: 980,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-soft01.webp?alt=media",
                            alias: "fm01-mouth-soft01",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                ],
                width: 546,
                height: 980,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: -546,
                    y: 100,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: -546,
                y: 100,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
                loadIsStarted: false,
            },
        },
        {
            kind: "N",
            path: ["canvas", "elements", "steph"],
            rhs: {
                pixivnId: "ImageContainer",
                elements: [
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 525,
                        height: 969,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-body.webp?alt=media",
                            alias: "fm02-body",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 525,
                        height: 969,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-eyes-nervous.webp?alt=media",
                            alias: "fm02-eyes-nervous",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 525,
                        height: 969,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-mouth-nervous00.webp?alt=media",
                            alias: "fm02-mouth-nervous00",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                ],
                width: 525,
                height: 969,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: 2445,
                    y: 111,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: 2445,
                y: 111,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
                loadIsStarted: false,
            },
        },
        {
            kind: "E",
            path: ["canvas", "stage", "width"],
            lhs: 1920,
            rhs: 3516,
        },
        {
            kind: "A",
            path: ["canvas", "elementAliasesOrder"],
            index: 3,
            item: {
                kind: "N",
                rhs: "steph",
            },
        },
        {
            kind: "A",
            path: ["canvas", "elementAliasesOrder"],
            index: 2,
            item: {
                kind: "N",
                rhs: "sly",
            },
        },
        {
            kind: "N",
            path: ["canvas", "tickersOnPause", "sly"],
            rhs: {
                tickerIdsExcluded: ["41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd"],
            },
        },
        {
            kind: "N",
            path: ["canvas", "tickersOnPause", "steph"],
            rhs: {
                tickerIdsExcluded: ["170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7"],
            },
        },
        {
            kind: "A",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds"],
            index: 1,
            item: {
                kind: "N",
                rhs: {
                    id: "170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7",
                },
            },
        },
        {
            kind: "A",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds"],
            index: 0,
            item: {
                kind: "N",
                rhs: {
                    id: "41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd",
                },
            },
        },
        {
            kind: "N",
            path: ["sound", "filters"],
            rhs: undefined,
        },
        {
            kind: "E",
            path: ["labelIndex"],
            lhs: 13,
            rhs: 14,
        },
        {
            kind: "E",
            path: ["openedLabels", 0, "currentStepIndex"],
            lhs: 13,
            rhs: 14,
        },
    ];
    let differencesNew: Difference[] = [
        {
            path: ["storage", 0, "value", "text"],
            type: "CHANGE",
            value: "I just wanted to see what the new guy was like.",
            oldValue: "I know you're both watching, come on out already!",
        },
        {
            path: ["storage", 0, "value", "character"],
            type: "CHANGE",
            value: "sly",
            oldValue: "james",
        },
        {
            path: ["storage", 1, "value"],
            type: "CHANGE",
            value: 14,
            oldValue: 13,
        },
        {
            path: ["storage", 3, "value", "start", "biggestStep"],
            type: "CHANGE",
            value: 14,
            oldValue: 13,
        },
        {
            type: "CREATE",
            path: ["canvas", "tickers", "41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd"],
            value: {
                id: "MoveTicker",
                args: {
                    direction: "right",
                    speed: 300,
                    tickerAliasToResume: ["sly"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.2,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["sly"],
            },
        },
        {
            type: "CREATE",
            path: ["canvas", "tickers", "170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7"],
            value: {
                id: "MoveTicker",
                args: {
                    direction: "left",
                    speed: 300,
                    tickerAliasToResume: ["steph"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.8,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["steph"],
            },
        },
        {
            path: ["canvas", "elements", "james", "elements", 2, "textureData", "url"],
            type: "CHANGE",
            value: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy00.webp?alt=media",
            oldValue:
                "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy01.webp?alt=media",
        },
        {
            path: ["canvas", "elements", "james", "elements", 2, "textureData", "alias"],
            type: "CHANGE",
            value: "m01-mouth-annoy00",
            oldValue: "m01-mouth-annoy01",
        },
        {
            type: "CREATE",
            path: ["canvas", "elements", "sly"],
            value: {
                pixivnId: "ImageContainer",
                elements: [
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 546,
                        height: 980,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-body.webp?alt=media",
                            alias: "fm01-body",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 546,
                        height: 980,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-wow.webp?alt=media",
                            alias: "fm01-eyes-wow",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 546,
                        height: 980,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-soft01.webp?alt=media",
                            alias: "fm01-mouth-soft01",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                ],
                width: 546,
                height: 980,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: -546,
                    y: 100,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: -546,
                y: 100,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
                loadIsStarted: false,
            },
        },
        {
            type: "CREATE",
            path: ["canvas", "elements", "steph"],
            value: {
                pixivnId: "ImageContainer",
                elements: [
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 525,
                        height: 969,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-body.webp?alt=media",
                            alias: "fm02-body",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 525,
                        height: 969,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-eyes-nervous.webp?alt=media",
                            alias: "fm02-eyes-nervous",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                    {
                        pixivnId: "Image",
                        elements: [],
                        width: 525,
                        height: 969,
                        isRenderGroup: false,
                        blendMode: "inherit",
                        tint: 16777215,
                        alpha: 1,
                        angle: 0,
                        renderable: true,
                        rotation: 0,
                        scale: {
                            x: 1,
                            y: 1,
                        },
                        pivot: {
                            x: 0,
                            y: 0,
                        },
                        position: {
                            x: 0,
                            y: 0,
                        },
                        skew: {
                            x: 0,
                            y: 0,
                        },
                        visible: true,
                        x: 0,
                        y: 0,
                        eventMode: "passive",
                        interactive: false,
                        interactiveChildren: true,
                        hitArea: null,
                        textureData: {
                            url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-mouth-nervous00.webp?alt=media",
                            alias: "fm02-mouth-nervous00",
                        },
                        anchor: {
                            x: 0,
                            y: 0,
                        },
                        roundPixels: false,
                        onEvents: {},
                        loadIsStarted: false,
                    },
                ],
                width: 525,
                height: 969,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: 2445,
                    y: 111,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: 2445,
                y: 111,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
                loadIsStarted: false,
            },
        },
        {
            path: ["canvas", "stage", "width"],
            type: "CHANGE",
            value: 3516,
            oldValue: 1920,
        },
        {
            type: "CREATE",
            path: ["canvas", "elementAliasesOrder", 2],
            value: "sly",
        },
        {
            type: "CREATE",
            path: ["canvas", "elementAliasesOrder", 3],
            value: "steph",
        },
        {
            type: "CREATE",
            path: ["canvas", "tickersOnPause", "sly"],
            value: {
                tickerIdsExcluded: ["41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd"],
            },
        },
        {
            type: "CREATE",
            path: ["canvas", "tickersOnPause", "steph"],
            value: {
                tickerIdsExcluded: ["170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7"],
            },
        },
        {
            type: "CREATE",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds", 0],
            value: {
                id: "41a8f13900d641b94fc1d547c129f5f2f3d9766b_yf86kd",
            },
        },
        {
            type: "CREATE",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds", 1],
            value: {
                id: "170deb3d2b65d1d470ad0e59be142bcaf6933560_baof7",
            },
        },
        {
            type: "CREATE",
            path: ["sound", "filters"],
            value: undefined,
        },
        {
            path: ["labelIndex"],
            type: "CHANGE",
            value: 14,
            oldValue: 13,
        },
        {
            path: ["openedLabels", 0, "currentStepIndex"],
            type: "CHANGE",
            value: 14,
            oldValue: 13,
        },
    ];
    let resOld = restoreDiffChanges(data, differencesOld);
    let resNew = restoreDiffChanges(data, differencesNew);

    let resExpected = {
        path: "/narration",
        storage: [
            {
                key: "___current_dialogue_memory___",
                value: {
                    text: "I know you're both watching, come on out already!",
                    character: "james",
                },
            },
            {
                key: "___last_dialogue_added_in_step_memory___",
                value: 13,
            },
            {
                key: "___flags___",
                value: [],
            },
            {
                key: "___opened_labels_counter___",
                value: {
                    start: {
                        biggestStep: 13,
                        openCount: 1,
                    },
                },
            },
            {
                key: "_input_value_",
                value: "Peter",
            },
            {
                key: "___last_input_added_in_step_memory___",
                value: 5,
            },
            {
                key: "___character___",
                value: {
                    mc: {
                        name: "Peter",
                    },
                },
            },
        ],
        canvas: {
            tickers: {},
            tickersSteps: {},
            elements: {
                bg: {
                    pixivnId: "Image",
                    elements: [],
                    width: 1920,
                    height: 1080,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 0,
                        y: 0,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 0,
                    y: 0,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    textureData: {
                        url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fbg01-hallway.webp?alt=media",
                        alias: "bg01-hallway",
                    },
                    anchor: {
                        x: 0,
                        y: 0,
                    },
                    roundPixels: false,
                    onEvents: {},
                    loadIsStarted: false,
                },
                james: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-body.webp?alt=media",
                                alias: "m01-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-eyes-annoy.webp?alt=media",
                                alias: "m01-eyes-annoy",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy01.webp?alt=media",
                                alias: "m01-mouth-annoy01",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 495,
                    height: 977,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 712.5,
                        y: 103,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 712.5,
                    y: 103,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    align: {
                        x: 0.5,
                        y: 1,
                    },
                    loadIsStarted: false,
                },
            },
            stage: {
                pixivnId: "Container",
                elements: [],
                width: 1920,
                height: 1080,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: 0,
                    y: 0,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: 0,
                y: 0,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
            },
            elementAliasesOrder: ["bg", "james"],
            tickersOnPause: {},
            tickersToCompleteOnStepEnd: {
                tikersIds: [],
                stepAlias: [],
            },
        },
        sound: {
            soundsPlaying: {},
            soundAliasesOrder: [],
        },
        labelIndex: 13,
        openedLabels: [
            {
                label: "start",
                currentStepIndex: 13,
            },
        ],
    };
    expect(resOld).toEqual(resExpected);
    expect(resNew).toEqual(resExpected);
});

test("restoreDeepDiffChanges & restoreMicroDiffChanges 2", async () => {
    let data = {
        path: "/narration",
        storage: [
            {
                key: "___current_dialogue_memory___",
                value: {
                    text: "I just wanted to see what the new guy was like. Hey, you, Peter- be nice to our little brother,",
                    character: "sly",
                },
            },
            {
                key: "___last_dialogue_added_in_step_memory___",
                value: 15,
            },
            {
                key: "___flags___",
                value: [],
            },
            {
                key: "___opened_labels_counter___",
                value: {
                    start: {
                        biggestStep: 15,
                        openCount: 1,
                    },
                },
            },
            {
                key: "_input_value_",
                value: "Peter",
            },
            {
                key: "___last_input_added_in_step_memory___",
                value: 5,
            },
            {
                key: "___character___",
                value: {
                    mc: {
                        name: "Peter",
                    },
                },
            },
        ],
        canvas: {
            tickers: {},
            tickersSteps: {},
            elements: {
                bg: {
                    pixivnId: "Image",
                    elements: [],
                    width: 1920,
                    height: 1080,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 0,
                        y: 0,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 0,
                    y: 0,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    textureData: {
                        url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fbg01-hallway.webp?alt=media",
                        alias: "bg01-hallway",
                    },
                    anchor: {
                        x: 0,
                        y: 0,
                    },
                    roundPixels: false,
                    onEvents: {},
                    loadIsStarted: false,
                },
                james: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-body.webp?alt=media",
                                alias: "m01-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-eyes-annoy.webp?alt=media",
                                alias: "m01-eyes-annoy",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy00.webp?alt=media",
                                alias: "m01-mouth-annoy00",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 495,
                    height: 977,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 712.5,
                        y: 103,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 712.5,
                    y: 103,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    align: {
                        x: 0.5,
                        y: 1,
                    },
                    loadIsStarted: false,
                },
                sly: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-body.webp?alt=media",
                                alias: "fm01-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-upset.webp?alt=media",
                                alias: "fm01-eyes-upset",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-smile01.webp?alt=media",
                                alias: "fm01-mouth-smile01",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 546,
                    height: 980,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 274.8,
                        y: 100,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 274.8,
                    y: 100,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    align: {
                        x: 0.2,
                        y: 1,
                    },
                    loadIsStarted: false,
                },
                steph: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-body.webp?alt=media",
                                alias: "fm02-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-eyes-nervous.webp?alt=media",
                                alias: "fm02-eyes-nervous",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-mouth-nervous00.webp?alt=media",
                                alias: "fm02-mouth-nervous00",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 525,
                    height: 969,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 1116,
                        y: 111,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 1116,
                    y: 111,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    align: {
                        x: 0.8,
                        y: 1,
                    },
                    loadIsStarted: false,
                },
            },
            stage: {
                pixivnId: "Container",
                elements: [],
                width: 1920,
                height: 1080,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: 0,
                    y: 0,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: 0,
                y: 0,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
            },
            elementAliasesOrder: ["bg", "james", "sly", "steph"],
            tickersOnPause: {},
            tickersToCompleteOnStepEnd: {
                tikersIds: [],
                stepAlias: [],
            },
        },
        sound: {
            soundsPlaying: {},
            soundAliasesOrder: [],
        },
        labelIndex: 15,
        openedLabels: [
            {
                label: "start",
                currentStepIndex: 15,
            },
        ],
    };
    let differencesOld: deepDiff.Diff<any, any>[] = [
        {
            kind: "E",
            path: ["storage", 3, "value", "start", "biggestStep"],
            lhs: 14,
            rhs: 15,
        },
        {
            kind: "E",
            path: ["storage", 1, "value"],
            lhs: 14,
            rhs: 15,
        },
        {
            kind: "E",
            path: ["storage", 0, "value", "text"],
            lhs: "I just wanted to see what the new guy was like.",
            rhs: "I just wanted to see what the new guy was like. Hey, you, Peter- be nice to our little brother,",
        },
        {
            kind: "D",
            path: ["canvas", "tickers", "41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855"],
            lhs: {
                id: "MoveTicker",
                args: {
                    direction: "right",
                    speed: 300,
                    tickerAliasToResume: ["sly"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.2,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["sly"],
            },
        },
        {
            kind: "D",
            path: ["canvas", "tickers", "170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux"],
            lhs: {
                id: "MoveTicker",
                args: {
                    direction: "left",
                    speed: 300,
                    tickerAliasToResume: ["steph"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.8,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["steph"],
            },
        },
        {
            kind: "E",
            path: ["canvas", "elements", "sly", "elements", 2, "textureData", "url"],
            lhs: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-soft01.webp?alt=media",
            rhs: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-smile01.webp?alt=media",
        },
        {
            kind: "E",
            path: ["canvas", "elements", "sly", "elements", 2, "textureData", "alias"],
            lhs: "fm01-mouth-soft01",
            rhs: "fm01-mouth-smile01",
        },
        {
            kind: "E",
            path: ["canvas", "elements", "sly", "elements", 1, "textureData", "url"],
            lhs: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-wow.webp?alt=media",
            rhs: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-upset.webp?alt=media",
        },
        {
            kind: "E",
            path: ["canvas", "elements", "sly", "elements", 1, "textureData", "alias"],
            lhs: "fm01-eyes-wow",
            rhs: "fm01-eyes-upset",
        },
        {
            kind: "E",
            path: ["canvas", "elements", "sly", "position", "x"],
            lhs: -546,
            rhs: 274.8,
        },
        {
            kind: "E",
            path: ["canvas", "elements", "sly", "x"],
            lhs: -546,
            rhs: 274.8,
        },
        {
            kind: "N",
            path: ["canvas", "elements", "sly", "align"],
            rhs: {
                x: 0.2,
                y: 1,
            },
        },
        {
            kind: "E",
            path: ["canvas", "elements", "steph", "position", "x"],
            lhs: 2445,
            rhs: 1116,
        },
        {
            kind: "E",
            path: ["canvas", "elements", "steph", "x"],
            lhs: 2445,
            rhs: 1116,
        },
        {
            kind: "N",
            path: ["canvas", "elements", "steph", "align"],
            rhs: {
                x: 0.8,
                y: 1,
            },
        },
        {
            kind: "E",
            path: ["canvas", "stage", "width"],
            lhs: 3516,
            rhs: 1920,
        },
        {
            kind: "D",
            path: ["canvas", "tickersOnPause", "sly"],
            lhs: {
                tickerIdsExcluded: ["41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855"],
            },
        },
        {
            kind: "D",
            path: ["canvas", "tickersOnPause", "steph"],
            lhs: {
                tickerIdsExcluded: ["170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux"],
            },
        },
        {
            kind: "A",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds"],
            index: 1,
            item: {
                kind: "D",
                lhs: {
                    id: "170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux",
                },
            },
        },
        {
            kind: "A",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds"],
            index: 0,
            item: {
                kind: "D",
                lhs: {
                    id: "41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855",
                },
            },
        },
        {
            kind: "N",
            path: ["sound", "filters"],
            rhs: undefined,
        },
        {
            kind: "E",
            path: ["labelIndex"],
            lhs: 14,
            rhs: 15,
        },
        {
            kind: "E",
            path: ["openedLabels", 0, "currentStepIndex"],
            lhs: 14,
            rhs: 15,
        },
    ];
    let differencesNew: Difference[] = [
        {
            path: ["storage", 0, "value", "text"],
            type: "CHANGE",
            value: "I just wanted to see what the new guy was like. Hey, you, Peter- be nice to our little brother,",
            oldValue: "I just wanted to see what the new guy was like.",
        },
        {
            path: ["storage", 1, "value"],
            type: "CHANGE",
            value: 15,
            oldValue: 14,
        },
        {
            path: ["storage", 3, "value", "start", "biggestStep"],
            type: "CHANGE",
            value: 15,
            oldValue: 14,
        },
        {
            type: "REMOVE",
            path: ["canvas", "tickers", "41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855"],
            oldValue: {
                id: "MoveTicker",
                args: {
                    direction: "right",
                    speed: 300,
                    tickerAliasToResume: ["sly"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.2,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["sly"],
            },
        },
        {
            type: "REMOVE",
            path: ["canvas", "tickers", "170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux"],
            oldValue: {
                id: "MoveTicker",
                args: {
                    direction: "left",
                    speed: 300,
                    tickerAliasToResume: ["steph"],
                    aliasToRemoveAfter: [],
                    destination: {
                        x: 0.8,
                        y: 1,
                        type: "align",
                    },
                    startOnlyIfHaveTexture: true,
                },
                canvasElementAliases: ["steph"],
            },
        },
        {
            path: ["canvas", "elements", "sly", "elements", 1, "textureData", "url"],
            type: "CHANGE",
            value: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-upset.webp?alt=media",
            oldValue:
                "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-wow.webp?alt=media",
        },
        {
            path: ["canvas", "elements", "sly", "elements", 1, "textureData", "alias"],
            type: "CHANGE",
            value: "fm01-eyes-upset",
            oldValue: "fm01-eyes-wow",
        },
        {
            path: ["canvas", "elements", "sly", "elements", 2, "textureData", "url"],
            type: "CHANGE",
            value: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-smile01.webp?alt=media",
            oldValue:
                "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-soft01.webp?alt=media",
        },
        {
            path: ["canvas", "elements", "sly", "elements", 2, "textureData", "alias"],
            type: "CHANGE",
            value: "fm01-mouth-smile01",
            oldValue: "fm01-mouth-soft01",
        },
        {
            path: ["canvas", "elements", "sly", "position", "x"],
            type: "CHANGE",
            value: 274.8,
            oldValue: -546,
        },
        {
            path: ["canvas", "elements", "sly", "x"],
            type: "CHANGE",
            value: 274.8,
            oldValue: -546,
        },
        {
            type: "CREATE",
            path: ["canvas", "elements", "sly", "align"],
            value: {
                x: 0.2,
                y: 1,
            },
        },
        {
            path: ["canvas", "elements", "steph", "position", "x"],
            type: "CHANGE",
            value: 1116,
            oldValue: 2445,
        },
        {
            path: ["canvas", "elements", "steph", "x"],
            type: "CHANGE",
            value: 1116,
            oldValue: 2445,
        },
        {
            type: "CREATE",
            path: ["canvas", "elements", "steph", "align"],
            value: {
                x: 0.8,
                y: 1,
            },
        },
        {
            path: ["canvas", "stage", "width"],
            type: "CHANGE",
            value: 1920,
            oldValue: 3516,
        },
        {
            type: "REMOVE",
            path: ["canvas", "tickersOnPause", "sly"],
            oldValue: {
                tickerIdsExcluded: ["41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855"],
            },
        },
        {
            type: "REMOVE",
            path: ["canvas", "tickersOnPause", "steph"],
            oldValue: {
                tickerIdsExcluded: ["170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux"],
            },
        },
        {
            type: "REMOVE",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds", 0],
            oldValue: {
                id: "41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855",
            },
        },
        {
            type: "REMOVE",
            path: ["canvas", "tickersToCompleteOnStepEnd", "tikersIds", 1],
            oldValue: {
                id: "170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux",
            },
        },
        {
            type: "CREATE",
            path: ["sound", "filters"],
            value: undefined,
        },
        {
            path: ["labelIndex"],
            type: "CHANGE",
            value: 15,
            oldValue: 14,
        },
        {
            path: ["openedLabels", 0, "currentStepIndex"],
            type: "CHANGE",
            value: 15,
            oldValue: 14,
        },
    ];
    let resOld = restoreDiffChanges(data, differencesOld);
    let resNew = restoreDiffChanges(data, differencesNew);

    let resExpected = {
        path: "/narration",
        storage: [
            {
                key: "___current_dialogue_memory___",
                value: {
                    text: "I just wanted to see what the new guy was like.",
                    character: "sly",
                },
            },
            {
                key: "___last_dialogue_added_in_step_memory___",
                value: 14,
            },
            {
                key: "___flags___",
                value: [],
            },
            {
                key: "___opened_labels_counter___",
                value: {
                    start: {
                        biggestStep: 14,
                        openCount: 1,
                    },
                },
            },
            {
                key: "_input_value_",
                value: "Peter",
            },
            {
                key: "___last_input_added_in_step_memory___",
                value: 5,
            },
            {
                key: "___character___",
                value: {
                    mc: {
                        name: "Peter",
                    },
                },
            },
        ],
        canvas: {
            tickers: {
                "41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855": {
                    id: "MoveTicker",
                    args: {
                        direction: "right",
                        speed: 300,
                        tickerAliasToResume: ["sly"],
                        aliasToRemoveAfter: [],
                        destination: {
                            x: 0.2,
                            y: 1,
                            type: "align",
                        },
                        startOnlyIfHaveTexture: true,
                    },
                    canvasElementAliases: ["sly"],
                },
                "170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux": {
                    id: "MoveTicker",
                    args: {
                        direction: "left",
                        speed: 300,
                        tickerAliasToResume: ["steph"],
                        aliasToRemoveAfter: [],
                        destination: {
                            x: 0.8,
                            y: 1,
                            type: "align",
                        },
                        startOnlyIfHaveTexture: true,
                    },
                    canvasElementAliases: ["steph"],
                },
            },
            tickersSteps: {},
            elements: {
                bg: {
                    pixivnId: "Image",
                    elements: [],
                    width: 1920,
                    height: 1080,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 0,
                        y: 0,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 0,
                    y: 0,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    textureData: {
                        url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fbg01-hallway.webp?alt=media",
                        alias: "bg01-hallway",
                    },
                    anchor: {
                        x: 0,
                        y: 0,
                    },
                    roundPixels: false,
                    onEvents: {},
                    loadIsStarted: false,
                },
                james: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-body.webp?alt=media",
                                alias: "m01-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-eyes-annoy.webp?alt=media",
                                alias: "m01-eyes-annoy",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 495,
                            height: 977,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Fm01%2Fm01-mouth-annoy00.webp?alt=media",
                                alias: "m01-mouth-annoy00",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 495,
                    height: 977,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 712.5,
                        y: 103,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 712.5,
                    y: 103,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    align: {
                        x: 0.5,
                        y: 1,
                    },
                    loadIsStarted: false,
                },
                sly: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-body.webp?alt=media",
                                alias: "fm01-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-eyes-wow.webp?alt=media",
                                alias: "fm01-eyes-wow",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 546,
                            height: 980,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm01%2Ffm01-mouth-soft01.webp?alt=media",
                                alias: "fm01-mouth-soft01",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 546,
                    height: 980,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: -546,
                        y: 100,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: -546,
                    y: 100,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    loadIsStarted: false,
                },
                steph: {
                    pixivnId: "ImageContainer",
                    elements: [
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-body.webp?alt=media",
                                alias: "fm02-body",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-eyes-nervous.webp?alt=media",
                                alias: "fm02-eyes-nervous",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                        {
                            pixivnId: "Image",
                            elements: [],
                            width: 525,
                            height: 969,
                            isRenderGroup: false,
                            blendMode: "inherit",
                            tint: 16777215,
                            alpha: 1,
                            angle: 0,
                            renderable: true,
                            rotation: 0,
                            scale: {
                                x: 1,
                                y: 1,
                            },
                            pivot: {
                                x: 0,
                                y: 0,
                            },
                            position: {
                                x: 0,
                                y: 0,
                            },
                            skew: {
                                x: 0,
                                y: 0,
                            },
                            visible: true,
                            x: 0,
                            y: 0,
                            eventMode: "passive",
                            interactive: false,
                            interactiveChildren: true,
                            hitArea: null,
                            textureData: {
                                url: "https://firebasestorage.googleapis.com/v0/b/pixi-vn.appspot.com/o/public%2Fbreakdown%2Ffm02%2Ffm02-mouth-nervous00.webp?alt=media",
                                alias: "fm02-mouth-nervous00",
                            },
                            anchor: {
                                x: 0,
                                y: 0,
                            },
                            roundPixels: false,
                            onEvents: {},
                            loadIsStarted: false,
                        },
                    ],
                    width: 525,
                    height: 969,
                    isRenderGroup: false,
                    blendMode: "inherit",
                    tint: 16777215,
                    alpha: 1,
                    angle: 0,
                    renderable: true,
                    rotation: 0,
                    scale: {
                        x: 1,
                        y: 1,
                    },
                    pivot: {
                        x: 0,
                        y: 0,
                    },
                    position: {
                        x: 2445,
                        y: 111,
                    },
                    skew: {
                        x: 0,
                        y: 0,
                    },
                    visible: true,
                    x: 2445,
                    y: 111,
                    eventMode: "passive",
                    interactive: false,
                    interactiveChildren: true,
                    hitArea: null,
                    loadIsStarted: false,
                },
            },
            stage: {
                pixivnId: "Container",
                elements: [],
                width: 3516,
                height: 1080,
                isRenderGroup: false,
                blendMode: "inherit",
                tint: 16777215,
                alpha: 1,
                angle: 0,
                renderable: true,
                rotation: 0,
                scale: {
                    x: 1,
                    y: 1,
                },
                pivot: {
                    x: 0,
                    y: 0,
                },
                position: {
                    x: 0,
                    y: 0,
                },
                skew: {
                    x: 0,
                    y: 0,
                },
                visible: true,
                x: 0,
                y: 0,
                eventMode: "passive",
                interactive: false,
                interactiveChildren: true,
                hitArea: null,
            },
            elementAliasesOrder: ["bg", "james", "sly", "steph"],
            tickersOnPause: {
                sly: {
                    tickerIdsExcluded: ["41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855"],
                },
                steph: {
                    tickerIdsExcluded: ["170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux"],
                },
            },
            tickersToCompleteOnStepEnd: {
                tikersIds: [
                    {
                        id: "41a8f13900d641b94fc1d547c129f5f2f3d9766b_ov855",
                    },
                    {
                        id: "170deb3d2b65d1d470ad0e59be142bcaf6933560_ax0yux",
                    },
                ],
                stepAlias: [],
            },
        },
        sound: {
            soundsPlaying: {},
            soundAliasesOrder: [],
        },
        labelIndex: 14,
        openedLabels: [
            {
                label: "start",
                currentStepIndex: 14,
            },
        ],
    };
    expect(resOld).toEqual(resExpected);
    expect(resNew).toEqual(resExpected);
});
