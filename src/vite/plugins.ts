...
export interface VitePluginPixivnOptions {
     ...
     * or both).
     *
     * The generated arrays can be used for runtime validation (e.g. passed to
     * `z.enum`) so that character and label ID checks are backed by the actual
     * registry, not just TypeScript types.
     *
     ...
     * The path may be absolute or relative to Vite `root`.
     *
     * @example "./src/pixi-vn-list.gen.ts"
     */
    listFilePath?: string;
}
...
export function vitePluginPixivn(options?: VitePluginPixivnOptions): Plugin {
    ...
    /** Returns the absolute path of the generated type file, or null if not configured. */
    ...
    /** Returns the absolute path of the generated list file, or null if not configured. */
     ...
     * `PixivnCharacterIds` and `PixivnLabelIds` with the current known IDs.
     */
    ...
    /**
     * Writes the auto-generated TypeScript file that exports runtime `as const` arrays
     * of all currently known character and label IDs.
     */
    function generateListFile(
        ...
        const lines = [
            ...
            "// This file is auto-generated by @drincs/pixi-vn vite plugin. Do not edit manually.",
            "",
            `export const characterIds = ${JSON.stringify(charIds)} as const;`,
            `export const labelIds = ${JSON.stringify(labelIds)} as const;`,
            "",
        ];
        writeFileSync(filePath, lines.join("\n"), "utf-8");
    }

    /** Regenerates the list file if configured. Errors are logged and swallowed. */
    function tryGenerateListFile(): void {
    ...
    /** Regenerates the type file if configured. Errors are logged and swallowed. */
    function tryGenerateTypeFile(): void {
    ...
    return {
        ...
        hotUpdate({ file, server }) {
            // Prevent HMR for the auto-generated files so that regenerating
            // them never causes a full-page reload.
            const absLabelTypePath = getAbsTypeFilePath();
