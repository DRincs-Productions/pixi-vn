...
/**
 * Virtual module id for the {@link VitePluginPixivnOptions.autoRegisterWorker} auto-injection.
 * Same `/@id/...` `index.html` trick as {@link VITE_PIXIVN_TESTING_VIRTUAL_ID} - see its doc
 * comment for why. Unlike testing, this one is also injected during `vite build`: the worker is
 * a real runtime feature meant to ship, not a dev-only hook.
 */
const VITE_PIXIVN_WORKER_REGISTER_VIRTUAL_ID = "virtual:pixi-vn-worker-register";
const RESOLVED_VITE_PIXIVN_WORKER_REGISTER_VIRTUAL_ID = `\0${VITE_PIXIVN_WORKER_REGISTER_VIRTUAL_ID}`;
...
export interface VitePluginPixivnOptions {
     ...
     * {@link typeFilePath} there's nothing to regenerate on content reload). Wire it up with
     ...
     * The path may be absolute or relative to Vite `root`.
     *
     * @example "./src/pixi-vn.worker.gen.ts"
     */
    workerFilePath?: string;

    /**
     * Auto-registers the worker generated by {@link VitePluginPixivnOptions.workerFilePath} by
     ...
     * how {@link VitePluginPixivnOptions.testing} auto-injects its own bridge.
     *
     ...
     *
     * Requires {@link VitePluginPixivnOptions.workerFilePath} to also be set; a project that needs
 ...
 *
 * When {@link VitePluginPixivnOptions.workerFilePath} is provided, the plugin writes a Worker
 ...
 * {@link VitePluginPixivnOptions.autoRegisterWorker} to also skip the manual
...
export function vitePluginPixivn(options?: VitePluginPixivnOptions): Plugin {
    ...
    /** Returns the absolute path of the generated worker file, or null if not configured. */
    function getAbsWorkerFilePath(): string | null {
        if (!options?.workerFilePath || !resolvedConfig) return null;
        const p = options.workerFilePath;
        return isAbsolute(p) ? p : resolve(resolvedConfig.root, p);
    }

    /**
     * Writes the auto-generated Worker entry file (see
     * {@link VitePluginPixivnOptions.workerFilePath}'s doc comment). Static content - called once,
     * not on every content reload like {@link tryGenerateKeysFile}.
     */
    function tryGenerateWorkerFile(): void {
        const absPath = getAbsWorkerFilePath();
        if (!absPath) return;
        try {
            ...
            resolvedConfig?.logger.error(
                `${PLUGIN_PREFIX} Failed to write worker file "${absPath}".`,
                {
                    error: error instanceof Error ? error : new Error(String(error)),
                    timestamp: true,
                },
    ...
    return {
        ...
        resolveId(id) {
            ...
            if (id === VITE_PIXIVN_WORKER_REGISTER_VIRTUAL_ID) {
                return RESOLVED_VITE_PIXIVN_WORKER_REGISTER_VIRTUAL_ID;
            }
        },
        ...
        load(id) {
            ...
            if (id === RESOLVED_VITE_PIXIVN_WORKER_REGISTER_VIRTUAL_ID) {
                const absWorkerPath = getAbsWorkerFilePath();
                if (!absWorkerPath) return "";
                return [
                    `import { Game } from "@drincs/pixi-vn";`,
                    `import PixivnWorker from ${JSON.stringify(`${absWorkerPath}?worker`)};`,
                    `Game.worker.register(new PixivnWorker());`,
                    "",
                ].join("\n");
            }
        ...
        transformIndexHtml() {
            ...
            // Unlike testing above, not gated on `command === "serve"` - see
            // VitePluginPixivnOptions.autoRegisterWorker's doc comment for why.
            if (options?.autoRegisterWorker && getAbsWorkerFilePath()) {
                tags.push({
                    tag: "script",
                    attrs: {
                        type: "module",
                        src: `/@id/${VITE_PIXIVN_WORKER_REGISTER_VIRTUAL_ID}`,
                    },
                    injectTo: "head-prepend",
                });
            }
        ...
        configResolved(config) {
            ...
            tryGenerateWorkerFile();
            if (options?.autoRegisterWorker && !getAbsWorkerFilePath()) {
                resolvedConfig.logger.warn(
                    `${PLUGIN_PREFIX} "autoRegisterWorker" has no effect without "workerFilePath" - set both to auto-register the generated worker.`,
                    { timestamp: true },
                );
            }
