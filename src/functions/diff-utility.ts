...
export function restoreDeepDiffChanges<T extends object = object>(data: T, differences: deepDiff.Diff<T, T>[]): T {
    ...
    differences.forEach((diff) => {
        ...
        if (diff.path && diff.path.length > 0) {
                ...
                if (diff.path && index === diff.path.length - 1) {
                    if (diff.kind === "E" || diff.kind === "D") {
                        ...
                        if (diff.item.kind === "N") {
                            ...
                            logger.warn("[Pixi’VN] Nested array found, skipping diff", diff);
                        } else {
                            logger.warn("[Pixi’VN] No array found, skipping diff", diff);
                        }
                    }
                } else {
            ...
            logger.warn("[Pixi’VN] No path found, skipping diff", diff);
        }
    });
    return result;
}
