...
export namespace Game {
     ...
     * - `setActions(actions)` is optional: if the app wraps `narration`/`stepHistory` in its own
     *   functions to handle UI-only concerns (a loading indicator, blocking while a menu is open,
     *   refreshing cached interface data, ...), it can register those functions so `continue()`,
     *   `goBack()`, `selectChoice()`, `start()`, `jump()` and `call()` drive the game through them
     *   instead of calling `narration`/`stepHistory` directly — same as a real player action would.
     ...
     */
    export const testing = createGameTesting(Game);
}
