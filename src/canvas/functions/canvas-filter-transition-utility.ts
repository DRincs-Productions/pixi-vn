 ...
 * local drawing coordinates (computed from `component`'s local bounds, see {@link applyWipeTransition}/
 * {@link applyIrisTransition}/{@link applySplitTransition}) still line up with `component` on screen
 * exactly as if it were a child.
 */
...
export function applyIrisTransition(
    component: CanvasBaseInterface<any>,
    config: IrisFilterConfig,
    value: number,
    ctx: FilterTransitionContext,
) {
    const progress = config.invert ? 1 - value : value;
...
export function applySplitTransition(
    component: CanvasBaseInterface<any>,
    ...
    const progress = config.invert ? 1 - value : value;
    // `progress` is 0 (hidden) -> 1 (fully shown), same convention as "wipe"/"iris". The two mask
    // panels start pulled apart past the component's own edges (no overlap, nothing visible) and slide
    // toward each other as progress grows, meeting exactly at the split line - "closed" - once progress
    // reaches 1, at which point they cover the component completely.
...
/**
 * Removes whatever mask {@link applyWipeTransition}/{@link applyIrisTransition}/{@link applySplitTransition}
 * attached, restoring the component to exactly the state it was in before the transition - transitions
 * must never permanently change unrelated component state once they complete.
 */
