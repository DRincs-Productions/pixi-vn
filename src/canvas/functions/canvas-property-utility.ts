...
export namespace CanvasPropertyUtility {
    ...
    export function getPointBySuperPoint(
        superPoint: { x: number; y: number },
        angle: number,
    ): { x: number; y: number } {
        angle = angle % 360;
        if (angle < 0) {
    ...
    export function getSuperWidth(canvasElement: PixiContainer): number {
        ...
        if (angle === 0 || angle === 180) {
            ...
            return (
                ...
                Math.abs(height * Math.sin((angle * Math.PI) / 180))
            );
        }
    }
    ...
    export function getSuperHeight(canvasElement: PixiContainer): number {
        ...
        if (angle === 0 || angle === 180) {
            ...
            return (
                ...
                Math.abs(width * Math.sin((angle * Math.PI) / 180))
            );
        }
    }
}
